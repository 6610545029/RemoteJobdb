...
cur.execute("CREATE TABLE JobAppli(id, jobSlug, jobTitle, companyName, jobIndustry, jobType, jobGeo, jobLevel, jobExcerpt, pubDate)")
cur.execute("""
...
for row in cur.execute("SELECT companyName FROM JobAppli WHERE jobIndustry = 'Software Engineering' AND (jobGeo = 'Anywhere' or 'Any')"):
  print(row)
